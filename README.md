...
We preformed OLS regession on different features such as adjusted budget. The dependant variables are adjusted revenues and ratings. We were also interested in the R-squared and the significance of the regression's coefficients. We used the Monte-Carlo method to retrieve a solid distribution of the R-squared. 

### T-tests
We used T-tests to compare the performance metrics of the renowned and non renowned actors. The performance metrics are the average rating and the average adjusted revenues (generated by a given actor). We also used T-tests to conduct an analyze based on movie genre. This implied to investigate if the there is a statistically significant difference in performance metrics between the movies played by renowned actors and played by non renowned actors. 

### Random forest
We performed a random forest 
